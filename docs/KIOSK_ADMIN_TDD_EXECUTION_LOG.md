...
## Consolidation/correctness repair round

A follow-up directive audited current HEAD (`a704f57`, after the parallel "Product Editor Scoped Round" and taxonomy/notification work had merged) against a fresh, more granular checklist covering Media Picker duplication, Lean V2 semantic correctness, Auth/security, Catalog/Product/Variant correctness, Settings/Dashboard/Orders, and repo hygiene. Every finding below was independently reproduced against current code (not assumed from the directive's description) before being fixed. Representative RED→GREEN cycles:

**Customer visibility semantics.** RED: two new cases in `product-visibility.test.ts` — "keeps the Product customer-visible when only one of several active Variants is eligible" and "hides the Product when every active Variant is individually ineligible" — failed against `getProductVisibility()`, which required *every* active Variant to be individually eligible (`reasons.length === 0` across all of them) instead of Lean V2's actual `get_customer_catalog()` rule (a Product is visible with **at least one** eligible active Variant; one bad sibling only removes itself). GREEN: rewrote the function to compute `hasEligibleVariant` independently of `blockingReasons`, returning `{ isCustomerVisible: true, reasons: [] }` as soon as any Variant qualifies. All 3 pre-existing tests stayed green unmodified.

**Media Picker camera-failure retention.** RED: a new `MediaPickerDialog.test.tsx` case mocked a failing `onUpload`, captured a photo through the camera flow, clicked "Use photo", and asserted the captured-photo preview and "Retake photo" button remained on screen — it failed (the preview vanished) because the button's `onClick` unconditionally cleared `capturedPhoto` via `.then(() => setCapturedPhoto(null))` regardless of whether the upload actually succeeded. Verified genuinely RED by stashing the fix and re-running (fails), then restoring. GREEN: `handleFileChange` now returns whether it succeeded; the button only clears the preview on success.

**Product editor N+1 reads.** RED: a new `useProductEditorData.test.tsx` case with two Variants asserted `listVariantMediaCounts`/`listVariantOptionValuesForVariants` were each called exactly once with `['variant-1','variant-2']` — failed (2 calls each) against the existing `variantIds.map(id => repo.xxx(id))` pattern. GREEN: added batched repository methods (`.in('variant_id', variantIds)`, grouped in JS) and switched the hook to call them once per Product instead of once per Variant.

**Admin User create rollback dual failure.** RED (subagent): a new case mocked `deleteUser()` resolving with `{ error: {...} }` (not rejecting) after a profile-insert failure — failed because the code only wrapped the rollback in `try/catch`, which never observes a normally-resolved-but-failed call. GREEN: inspect the resolved `error` and combine both failure messages when the rollback itself also fails.

**Sign-out error contract.** RED (subagent): a new case had `supabase.auth.signOut()` *resolve* with `{ error: someError }` — failed because `signOutCurrentUser()` discarded the return value entirely, only a rejected promise was ever handled. GREEN: destructure and throw `error`.

**Dashboard exact counts.** RED (subagent): synthetic 1500/2000/1200-row catalogs proved `activeProductCount`/`variantCount`/`unavailableVariantCount` undercounted at exactly 1000 (the old `.limit(1000)`-then-count-in-memory pattern). GREEN: exact `count:'exact',head:true` queries for the three that can be expressed as a single filter; `lowStockCount` (needs a per-row threshold-with-fallback comparison PostgREST can't express as one filter) pages through every active Variant in bounded 1000-row batches until a short page, staying exact at any size.

**Test-isolation bug found and fixed along the way.** While re-verifying the catalog-taxonomy subagent's Option Value hard-delete work, `OptionLibraryPanel.test.tsx`'s "leaves the Option Value untouched when the delete confirmation is cancelled" failed intermittently when run as part of the full file but passed in isolation — not a production bug: the file had no `beforeEach(() => vi.clearAllMocks())`, so an earlier test's real `deleteOptionValue` call leaked into this test's `not.toHaveBeenCalled()` assertion. Fixed by adding the missing `beforeEach`.

**Full remaining coverage** (all also TDD RED→GREEN, evidence in `docs/PR6_FINAL_EXECUTION_TODO.md`'s updated FIX rows): Media Picker consolidation onto Brand/Category/Store-logo consumers and deletion of the superseded `MediaAssetPickerDialog`; app-wide unsaved-changes navigation guard (`UnsavedChangesGuardProvider`/`useUnsavedChangesGuard`/`GuardedLink`) covering sidebar and header Links, not just `beforeunload`; Brand reorder + reorder-concurrency guards across all four taxonomy entities; Option Value read-error-vs-empty-state distinction; searchable Category root picker (removed a hardcoded `pageSize: 200` cap); master-deactivation confirmation warnings; Option Value hard delete with a `23503` FK-guard fallback; Store Settings RHF/Zod + shared Media Picker migration (the previous `store-settings.schema.ts` was dead code with field names matching neither the record nor the DB columns); per-route login redirect + authenticated-Login-page redirect; a regression test proving the already-correct `proxy.ts` SSR cookie-refresh behavior; Orders bounded query + AudioContext reuse/cleanup; a shared `CompactPagination` component adopted on Products; Product search widened to Brand/keywords/Variant SKU/barcode.

**Verification.** Full `pnpm vitest run` — **490 tests / 132 files, all passing**. `pnpm tsc --noEmit` — 0 errors. `pnpm biome ci` — 0 errors, only pre-existing `useSortedClasses`/`noImgElement` nursery warnings untouched by this round. Pushed as 12 focused conventional commits to the existing `feat/lean-v2-admin-integration` branch (PR #6), each with its own scoped test/type/lint verification before pushing.

**No hosted DB or browser access this round.** This sandbox's egress proxy rejects the hosted Supabase host outright (`curl` → `CONNECT tunnel failed, response 403`), reconfirmed directly before writing this entry — the same denial documented earlier in this log via curl/Node/psql/Playwright. Every fix above is verified at the code/regression-test/type-check/lint level only; live/hosted/browser re-verification of this round's specific changes is required once network access is available, and must not be represented as already proven.

**Process note.** Mid-round, one dispatched subagent ran `git stash`/`git stash pop` in the shared (non-worktree-isolated) working tree while three other subagents plus this session had uncommitted work in progress, which briefly discarded roughly 45 files of in-flight changes across all of them. Recovered in full via `git fsck --unreachable` (stash operations leave dangling commit objects even after the stash entry itself is dropped/popped) — located the matching WIP commit, restored every affected file from it, reconciled the one file (`AdminShell.tsx`) that had a legitimate edit on both sides, and re-verified the full test suite/type-check/lint before committing. No work was ultimately lost, but this is recorded as a real incident, not implied to have gone smoothly.

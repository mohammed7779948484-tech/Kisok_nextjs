...
- [~] **UX-PRODUCT-01 — Dedicated Product Create/Edit/View pages.** Locale-aware `/products/create`, `/products/[id]`, and `/products/[id]/edit` routes plus `ProductEditorPage` replace the list’s modal entry points. Route/resource contracts and Product list navigation tests pass. Remaining: authenticated browser and unsaved-work verification in phase 9. Evidence: `product-routes.test.ts`, `resources.test.ts`, `ProductCatalogPanel.test.tsx`.
- [~] **UX-PRODUCT-02 — Real Variant/Flavor deletion.** Product Edit now provides confirmed hard delete, refresh, explicit history-blocked explanation, and separately confirmed deactivation fallback. Repository outcome tests pass; authenticated browser evidence remains pending. Evidence: `variant-delete.test.ts`, `ProductEditorPage.tsx`.
- [ ] **FIX-01 — Product Category hydration safety.** Reset on product change; explicitly represent loading/ready/error; block relation writes while unavailable; provide Retry; preserve truly empty assignments. Required regressions: rapid A→B switch, immediate save, failed read, successful empty read. Evidence: pending.
...
- [x] **FIX-03 — Draft-first Product/Variant creation.** Product and Variant repositories explicitly persist `is_active: false`; the dedicated editor presents activation state. Regressions pass. Evidence: `product-crud.test.ts`, `variant.test.ts`, `supabase.ts`.
- [ ] **FIX-04 — Inactive Brand/Category selector state.** Preserve `isActive`; default new selection to active masters; show existing inactive relations; warn for active Product dependencies that hide customer visibility. Evidence: pending.
...
- [ ] **FIX-07 — Product cover Media.** Create/Edit must preview, select, upload/register, remove/replace, and persist `cover_media_asset_id`. Evidence: pending.
...
- [ ] **FIX-10 — Variant Media production wiring.** Expose attach/upload, detach, reorder, primary selection, and distinct asset deletion in the real Product Edit variant flow. Evidence: pending.
...
- [ ] **FIX-12 — Search keywords.** Deliberately expose Product/Variant search keywords or implement/document an unambiguous derivation rule. Evidence: pending.
...
- [ ] **FIX-15 — Product mutation errors.** Make activation/deactivation and other mutations reliably visible and actionable. Evidence: pending.
- [ ] **FIX-16 — Product reference-data errors.** Provide reference data loading/error/retry state; never present a failed form as ready. Evidence: pending.
...
- [ ] **FIX-21 — Product create compensation/draft behaviour.** Surface cleanup-delete dual failure or intentionally retain an explicitly saved draft; never pretend a partial create is atomic. Evidence: pending.
...
- [ ] **FIX-32 — Media Library search/pagination/usage.** Provide scalable search, pagination, preview, usage inspection, reusable selection, and guarded delete. Evidence: pending.
...
- [ ] **FIX-35 — Media file validation.** Validate MIME/type, size, and reasonable image constraints before upload. Evidence: pending.
...
- [ ] **FIX-67 — Product design-system consistency.** Dedicated pages must use project primitives and semantic KISOK wrappers; no raw duplicative controls. Evidence: pending.
- [ ] **FIX-68 — Variant form architecture.** Use RHF, Zod, and project primitives. Evidence: pending.
- [ ] **FIX-69 — Option removal primitive.** Replace raw button with design-system control. Evidence: pending.
...
- [~] **FIX-79 — Authenticated browser acceptance.** Login, protected dashboard, Product list/Create/Edit routes, Product detail hydration, draft/visibility diagnostics, Settings recovery, and live zero-duration rejection were verified. The isolated Variant's destructive confirmation did not render reliably from browser click, and empty hosted data prevented complete media/order/inventory/users flows. Evidence: `PR6_BROWSER_EVIDENCE.md`.
...
- [~] **FIX-81 — Critical regression suite.** The current 360-test suite includes new coverage for draft defaults, variant hydration, duplicate combinations, hard-delete mapping, Product Category and primary-Media compensation, Cloudinary cleanup, real order arrays, Settings validation, Admin login network failure, logout failure, and Dashboard status truthfulness. Coverage still lacks live session refresh propagation, Admin rollback dual failure, inactive Brand visibility, and Refine constraint UI. Evidence: `pnpm validate` on 2026-08-27.
...
- [ ] **FIX-83 — Searchable shared Media picker.** Use the same scalable reusable picker across Brand, Category, Product cover, Variant, and Store logo. Evidence: pending.
- [ ] **FIX-84 — Explicit Product visibility diagnostic.** Clearly show Draft/Active, Customer visible/hidden, and causes such as inactive dependencies/no active valid Variant. Evidence: pending.
